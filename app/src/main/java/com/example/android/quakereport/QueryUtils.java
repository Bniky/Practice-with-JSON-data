...
public final class QueryUtils {
    ...
    public static ArrayList<EarthQuakeData> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject j = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray features = j.optJSONArray("features");

            Log.v("QueryUtill", "HERE");
            for(int i=0;i<features.length();i++){
                JSONObject objectF = features.getJSONObject(i);
                JSONObject properties = objectF.getJSONObject("properties");
                ...
                earthquakes.add(new EarthQuakeData(place, mag, time, url));
            }
